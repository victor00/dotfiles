...
- **Warp** pode organizar abas e panes nativamente para trabalho local curto. Seu
  tema em `~/.local/share/warp-terminal/themes/` é vinculado individualmente ao
  dotfiles para que o scanner encontre o YAML sem substituir o diretório do Warp;
  preferências internas, conta, histórico e estado não são versionados.
